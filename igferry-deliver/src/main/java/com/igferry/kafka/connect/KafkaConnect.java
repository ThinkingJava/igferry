...
import com.igferry.kafka.receive.ReceiveMsgListener;
...
public class KafkaConnect {
    ...
    public KafkaConnect(final DeliverKafkaConfig kafkaConfig) {
        this.isRunning = false;
        this.initKafkaConnect(kafkaConfig);
    }

    public void initKafkaConnect(final DeliverKafkaConfig kafkaConfig) {
    ...
    private Properties getProducerConfig(final DeliverKafkaConfig kafkaConfig) {
    ...
    private Properties getConsumerConfig(final DeliverKafkaConfig kafkaConfig) {
        final Properties props = new Properties();
        props.put("bootstrap.servers", kafkaConfig.getServers()); // 设置 Broker 的地址
    ...
    public synchronized void startListenerMQReceiveMsg(final DeliverKafkaConfig kafkaConfig, final KafkaConnect kafkaConnect) {
        if (!this.isRunning) {
            final Thread listenerThread = new Thread(new ReceiveMsgListener(this.getKafkaConsumer(), kafkaConfig, kafkaConnect));
    ...
    public void sendKafkaMsg(final DeliverKafkaConfig kafkaConfig, final Request request, final String kafkaMsgKey) {
        sendCoustomKafka(kafkaConfig.getProducerTopic(),kafkaMsgKey, JacksonUtil.toJson(request));
    }
    ...
    public void sendCoustomKafka(final String sendTopic, final String sendKey, final String value) {
        this.getProducer().send(new ProducerRecord(sendTopic, sendKey, value), (metadata, exception) -> {
            if (exception != null) {
                ...
                log.debug("Send topic [{}] key [{}] kafka message success: {}",sendTopic, sendKey, value);
            }
        });
    }
